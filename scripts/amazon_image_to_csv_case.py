...
USER_AGENTS = [
    # A small pool of common desktop Chrome UAs
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if response.status_code == 200:
                ...
                if not img_tag:
                    # Print the first 1200 characters of the HTML for debugging (likely a bot-check page)
...
def update_csv_with_images(csv_path, limit: int | None = None):
    ...
    for idx, row in df.iterrows():
        ...
        if isinstance(row.get('image_link', None), str) and row['image_link'].strip():
            continue
        search_query = f"{row['name']} PC case"
        print(f"Searching for: {search_query}")
        img_link = get_amazon_image_link(search_query)
        ...
        # Randomized polite delay between requests to reduce blocking
...
def main():
    # Prevent running multiple Amazon scrapers concurrently (very likely to trigger blocking)
    ...
    try:
        ...
        if env_limit and env_limit.isdigit():
            ...
            try:
                ...
                batch_limit = None
        update_csv_with_images("csv/case.csv", limit=batch_limit)
    finally:
        try:
            lock_file.unlink()
